...
On the main-process side, `electron/main.ts` initializes the database and registers the IPC handlers. The canonical handler implementations live in `electron/database/database.ts` (via `ipcMain.handle('database:*', ...)`) and perform all SQLite work (including migrations via `electron/database/db-migrate.ts` and `electron/database/migrations/`). Response envelopes for CRUD operations (e.g. `CreateResponse`, `UpdateResponse`) are standardized in shared types so the renderer and main process agree on a stable contract.
...
- **Source of truth: main-process handler registration**: `electron/main.ts`, `electron/database/database.ts` (`setupDatabaseHandlers`)
