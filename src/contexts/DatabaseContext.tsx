...
import { mapDbTagToTag, mapDbTagsToTags } from '@/mappers/tagMapper';
import { mapDbProjectToProject, mapDbProjectsToProjects } from '@/mappers/projectMapper';
import { mapDbSessionToSession, mapDbSessionsToSessions } from '@/mappers/sessionMapper';

const changesOnly = (result: { changes: number }): ChangesOnlyResponse => ({
...
export const DatabaseProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const getProject = async (projectId: number): Promise<Project> => {
    ...
    return mapDbProjectToProject(project);
  };

  const getAllProjects = async (): Promise<Project[]> => {
    try {
      const projects = await window.database.getProjects();
      return mapDbProjectsToProjects(projects);
    } catch (error) {
  ...
  const createProject = async (
    ...
    return mapDbProjectToProject(result.record);
  };
  ...
  const getSessions = async (): Promise<Session[]> => {
    try {
      ...
      return mapDbSessionsToSessions(sessions);
    } catch (error) {
  ...
  const createSession = async (projectId: number, notes?: string): Promise<Session> => {
    try {
      ...
      return mapDbSessionToSession(result.record);
    } catch (error) {
